...
| `src/sections/hero/HeroSection.tsx` (`h-[33vh]`, carousel) | Viewport-relative unit. `token-lint.mjs` has no vh/vw comparison logic: `toPx()` returns null for vh/vw, but only `em` is routed to `unverifiable[]`, so a vh value falls through to `violations[]` with `nearestAllowed: null`. Same class of tool gap as the already-handled `em` case, not invented spacing. | 2026-08-02 |
| `src/app/globals.css` (`.gradient-overlay::before` / `::after`) | The two `linear-gradient()` ramps are copied byte-for-byte from the live site's own stylesheet, so they are authored source values rather than guesses. 22 of the 26 stop colors flag because `tokens.lock.json`'s `colors` bucket only holds computed element colors (`color` / `background-color` / `border-color`) and never gradient stops, so a fade ramp can never resolve there. `#000` and `#0000000b` do match and pass cleanly. | 2026-08-02 |
